...
func main() {
	...
	n.Handle("send", s.handle_send)
	n.Handle("poll", s.handle_poll)
	n.Handle("commit_offsets", s.handle_commit_offset)
	n.Handle("list_committed_offsets", s.handle_list_committed_offsets)
...
func (s *State) handle_send(msg maelstrom.Message) error {
	...
	len := len(s.Logs[key]) + s.OffsetBase
...
func (s *State) handle_poll(msg maelstrom.Message) error {
...
func (s *State) handle_commit_offset(msg maelstrom.Message) error {
	var body OffsetReq
...
func (s *State) handle_list_committed_offsets(msg maelstrom.Message) error {
	var body KeysReq
	if err := json.Unmarshal(msg.Body, &body); err != nil {
		return err
	}
	...
	return s.Node.Reply(msg, result)
}
